...
def payment_page(update, context):
    ...
    if package[0][7]:
        ...
        if query.message.chat_id in [6458732795, 6450325872]:
            keyboard.append([InlineKeyboardButton("درگاه پرداخت زرین پال", callback_data=f"zarinpall_page_buy_{id_}")])
...
def payment_page_upgrade(update, context):
    ...
    if query.message.chat_id in [6458732795, 6450325872]:
        keyboard.append([InlineKeyboardButton("درگاه پرداخت زرین پال", callback_data=f"zarinpall_page_upgrade_{id_}")])
